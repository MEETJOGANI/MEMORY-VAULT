...
from memory_processor import (
    ...
    check_api_availability  # Import function to check API availability
)
...
# Define a function to check API status
...
# Add a refresh button to re-check API status
...
# Navigation
tab_options = ["Capture", "Recall", "Mind Map", "Time Capsule", "Story Book", "Summaries"]
...
# Load the new story generation function into app.py
from memory_processor import (
    process_memory,
    generate_memory_embedding,
    recall_memories,
...
elif st.session_state.active_tab == "Story Book":
    st.markdown("<div class='sub-header'>📖 Memory Story Book</div>", unsafe_allow_html=True)
    
    # Apply additional CSS for the book display
    st.markdown("""
    <style>
    .book-container {
        background-color: #ffffff;
    ...
    if st.session_state.memories:
        ...
        if available_memories:
            ...
            story_type = st.selectbox(
                "Select story style:",
                ["Narrative", "Memoir", "Adventure", "Reflective", "Poetic"]
            )
            
            # Sort memories by date
            sorted_memories = sorted(available_memories, key=lambda x: x.get('date', ''))
            
            # Generate story button
            if st.button("Generate Story Book"):
                with st.spinner("Creating your story book... This may take a moment as we weave your memories together."):
                    story = generate_story_from_memories(sorted_memories, story_type.lower())
                    
                    # Display the story in a book format
                    st.markdown(f"""
                    <div class="book-cover">
                        <h1>{story.get('title', 'Your Memory Story')}</h1>
                        <div class="book-cover-desc">{story.get('cover_description', 'A collection of your memories')}</div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    # Offer download as PDF option
                    st.info("In a future update, you'll be able to download your story as a PDF to keep or share!")
        else:
            st.info("No available memories to create a story. Add some memories first!")
    else:
        st.info("You don't have any memories yet. Start capturing memories to create your story book.")
