...
# Function to check API availability - this allows the app to re-check on demand
...
def generate_story_from_memories(memories, story_type="narrative"):
    """
    Generate a coherent story that connects multiple memories into a narrative.
    
    Args:
        memories (list): List of memory objects to include in the story
        story_type (str): Type of story to generate (e.g., "narrative", "memoir", "adventure")
        
    Returns:
        dict: Story object with title and content
    """
    if not memories:
        return {
            "title": "No Memories to Tell",
            "content": "Add some memories to create your story book.",
            "cover_description": "An empty book waiting to be filled"
        }
    
    # Basic story generator without API
    def basic_story_generator(memory_list):
        # Get text from memories
        memory_texts = [m.get('text', '') for m in memory_list]
        combined_text = " ".join(memory_texts)
        
        # Generate a simple title based on keywords
        words = combined_text.lower().split()
        common_words = [word for word in words if len(word) > 4 and words.count(word) > 1]
        
        if common_words:
            title_word = max(set(common_words), key=common_words.count)
            title = f"The {title_word.capitalize()} Chronicles"
        else:
            title = "My Memory Collection"
        
        # Create a simple intro
        intro = "Here begins the story of my memories, collected and preserved..."
        
        # Join memories with simple transitions
        content_parts = [intro]
        for i, memory in enumerate(memory_list):
            date = memory.get('date', '')[:10] if memory.get('date') else 'One day'
            emotion = memory.get('emotion', 'Neutral')
            text = memory.get('text', '')
            
            if i == 0:
                transition = f"On {date}, I felt {emotion.lower()} when:"
            else:
                transition = f"Later, on {date}, I felt {emotion.lower()} as:"
                
            content_parts.append(f"{transition} {text}")
            
        content_parts.append("And so the story continues, with more memories to come...")
        content = "\n\n".join(content_parts)
        
        return {
            "title": title,
            "content": content,
            "cover_description": f"A collection of memories featuring themes of {', '.join(set([m.get('emotion', 'experiences') for m in memory_list[:3]]))}"
        }
    
    try:
        if openai_available:
            # Prepare memories for API prompt
            memory_json = json.dumps([{
                ...
                'topics': m.get('topics', [])
            } for m in memories[:20]])  # Limit to first 20 memories to avoid token limits
            
            # Use OpenAI to generate a story connecting the memories
            response = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert storyteller and author. Your task is to weave memories into a "
                        f"coherent {story_type} that flows naturally. Generate an engaging title that captures "
                        "the essence of the memories, and create a narrative that connects them in a meaningful way. "
                        "Focus on emotions, people, locations, and themes that appear across multiple memories. "
                        "Also provide a brief cover description for the story. "
                        "Respond with a JSON object with the following structure: "
                        "{'title': 'Your Generated Title', 'content': 'Your story content...', 'cover_description': 'A brief description for the cover'}"
                    },
                    {"role": "user", "content": f"Here are the memories to include in the story: {memory_json}"}
                ],
            ...
            if 'title' not in result or 'content' not in result:
                return basic_story_generator(memories)
                
            return result
        else:
            # Use the basic story generator when API is not available
            return basic_story_generator(memories)
...
try:
    ...
    print(f"Error during initial API check: {e}")
